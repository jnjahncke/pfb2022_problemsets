...
if [ -e ./ecoli-12.5X.$jobid.meryl/merylIndex ] ; then
...
if [ -e ecoli-12.5X.$jobid.meryl.tar.gz ]; then
  exist1=true
else
  exist1=false
fi
...
/Users/student/pfb2022_problemsets/GenomeAssembly/canu-2.2/bin/meryl k=22 threads=4 memory=2 \
  count \
    segment=$jobid/01 ../../ecoli-12.5X.seqStore \
    output ./ecoli-12.5X.$jobid.meryl.WORKING \
&& \
mv -f ./ecoli-12.5X.$jobid.meryl.WORKING ./ecoli-12.5X.$jobid.meryl
